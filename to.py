...
def show_results():
    ...
    # Save data - WITH DUPLICATE CHECK
    student_data = {
        **st.session_state.student_info,
        "score": score,
    ...
    # Check if student already exists (by name, roll number and class)
    student_exists = False
    for i, student in enumerate(data["students"]):
        if (student["name"] == student_data["name"] and 
            student["roll_number"] == student_data["roll_number"] and 
            student["class"] == student_data["class"]):
            # Update existing record instead of creating new one
            data["students"][i] = student_data
            student_exists = True
            st.info("🔄 Your previous result has been updated!")
            break
...
def view_results():
    ...
    # Prepare data for table - show unique students only
    seen_students = set()
    table_data = []
    
    for student in data["students"]:
        student_key = (student["name"], student["roll_number"], student["class"])
    ...
    with col1:
        st.metric("Total Students", len(table_data))  # Unique students count
    with col2:
...
def admin_panel():
    ...
    if data["students"]:
        ...
        with col2:
            st.metric("Unique Students", unique_students_count)
        with col3:
...
def show_about():
    ...
    <div style='background: rgba(255, 107, 107, 0.1); padding: 30px; border-radius: 15px; border-left: 5px solid #FF6B6B;'>
        ...
        <h4 style='color: #1a1a1a; margin-top: 20px;'>🔄 Duplicate Prevention:</h4>
        <p style='color: #1a1a1a;'>The system automatically prevents duplicate entries by checking student name, roll number, and class. 
        If a student takes the test again, their previous result is updated instead of creating a new record.</p>
    </div>
    """, unsafe_allow_html=True)
